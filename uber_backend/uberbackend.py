...
#mongo_client = MongoClient("mongodb://Mahitha-Maddi:Mahitha%4042@cluster0-shard-00-00.1z0g8.mongodb.net/?ssl=true&ssl_cert_reqs=CERT_NONE")
app = Flask(__name__)
# CORS(app)
...
def validation2(user, source, destination, date):
    ...
    with mongo_client:
        ...
        mongo_collection = db['bookings']
        myquery1 = {"source": {"$regex": str(source)}, "destination": {"$regex": str(
            destination)}, "date": {"$regex": str(date)}, "user": {"$regex": str(user)}}
        cursor1 = dict()
    ...
    with mongo_client:
        ...
        try:
            ...
            mongo_collection.update_one({"_id": busid},
                                        {"$set": {"FA": FA, "FB": FB, "FC": FC, "SA": SA,
                                                  "SB": SB, "SC": SC, "TA": TA, "TB": TB, "TC": TC}},
                                        upsert=True)
            # , result.modified_count)
            print("...update_one() bus for reserved seats to mongo acknowledged:")
    ...
    with mongo_client:
        ...
        try:
            ...
            mongo_collection.update_one({"email": email},
                                        ...
                                        upsert=True)
            # , result.modified_count)
            print("...update_one() user forgot password:")
            return jsonify("successfully updated!")
        except Exception as e:
            print(e)
    return jsonify("successfully updated!")
...
def book_bus():
    ...
    booking = dict(source=source, passengers=passengers, seatsBooked=seatsBooked, destination=destination, busnumber=busnumber, user=user, numOfSeats=numOfSeats,
                   totalPrice=totalPrice, date=date, startTime=startTime, endTime=endTime, bookeddate=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), _id=str(ObjectId()))
...
def add_passenger():
    ...
    passenger = dict(bookingID=bookingID, seatNumber=seatNumber,
                     fullname=fullname, gender=gender, _id=str(ObjectId()))
...
def user_signup():
    ...
    if not howmany and not howmany2:
        user = dict(username=username, password=password, email=email,
                    dob=dob, contact=contact, _id=str(ObjectId()))
